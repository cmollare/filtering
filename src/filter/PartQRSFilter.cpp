#include "PartQRSFilter.h"

PartQRSFilter::PartQRSFilter(std::vector<S3DModel*> mods, std::vector<std::string> posNames, std::vector<std::vector<double> > jointsXYZPositions) : Filter(mods)
{
...
void PartQRSFilter::computeDistance()
{
...
void PartQRSFilter::computeDistance(int partition)
{
...
void PartQRSFilter::computeLikelihood()
{	
...
void PartQRSFilter::computeLikelihood(int partition)
{
	for (int i=0 ; i<mCurrentDistances.size() ; i++)
	{
	...
	for (int i=1 ; i<=partition ; i++)
	{
		...
		for (int i=0 ; i<mCurrentDistances.size() ; i++)
		{
			mCurrentLikelihood[i] *= exp(-abs(mCurrentDistances[i])*30);//20
		}
	}
}

void PartQRSFilter::step(std::vector<std::vector<double> > frame)
{
...
double PartQRSFilter::computeNeff()
{
	return 1./(mCurrentWeights.dot(mCurrentWeights));
}

void PartQRSFilter::updateWeights(int partition)
{
	double sum=0;
...
void PartQRSFilter::mapJointToObs(std::map<std::string, std::string> jointNameToPosName)
{
...
S3DModel* PartQRSFilter::computeMMSE()
{
